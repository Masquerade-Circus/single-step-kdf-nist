...
const getHashLength = (hash) => {
    switch (hash) {
        ...
        case "md5":
            return 16;
        default: {
            return crypto_1.default.createHash(hash).digest().length;
        }
    }
};
const intTo4Bytes = (counter) => {
    return utils_1.hexToBytes(counter.toString().padStart(7, "0"));
};
const checkOutLength = (outLengthBytes) => {
    if (outLengthBytes <= 0) {
        throw new IllegalArgumentException("outLengthBytes must be greatear than 0");
    }
};
/**
 ...
 */
exports.singleStepKDF = (hash, sharedSecretZ, outLengthBytes, fixedInfo) => {
    ...
    do {
        crypto_1.default
            ...
            .update(fixedInfo)
            .digest()
            .copy(buffer, 0, 0, reps === counter ? outLengthBytes - outputLenSum : digestByteLength);
        outputLenSum += digestByteLength;
    } while (counter++ < reps);
    return new Uint8Array(buffer.toJSON().data);
};
//# sourceMappingURL=single-step-kdf.js.map